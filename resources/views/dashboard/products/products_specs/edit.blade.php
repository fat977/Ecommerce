@extends('dashboard.layouts.master')
@section('title','Edit a product specs')

@section('content')
<div class="content-wrapper">
    <div class="row p-4">
        @include('dashboard.includes.message')
        <div class="col-12">
            <form action="{{ route('admin.spec.update',[$data->product_id,$data->spec_id]) }}" method="POST">
                @csrf
                <div class="col-6">
                    <label for="value">product</label>
                    <input type="text" name="product_id" class="form-control" value="{{ $data->product_id }}">
                    @error('product_id')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
                <div class="col-6">
                    <label for="name_en">Spec</label>
                    <input type="text" name="spec_id" class="form-control" value="{{ $data->spec_id }}">
                </div>
                <div class="col-6">
                    <label for="value">Value</label>
                    <input type="text" name="value" class="form-control" value="{{ $data->value}}">
                    @error('value')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
                <div class="form-row my-3">
                    <div class="col-2">
                        <button class="btn btn-primary w-100" value="index"> Update </button>
                    </div>
                </div>
            </form>
        </div>
    </div>
</div>
@endsection
