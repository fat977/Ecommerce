    ...
    <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown notification-dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right notification-count">
                ...
                @else
                    <span class="dropdown-item dropdown-header notification-count">There are no notifications !</span>
                @endif
                <div class="dropdown-divider"></div>
                @if (Auth::guard('admin')->user()->unreadNotifications->count() > 0)
                    @foreach (Auth::guard('admin')->user()->unreadNotifications as $notification)
                        <a href="{{ route('admin.orders.view',$notification->data['id']) }}" class="dropdown-item">
                            <i class="fas fa-file mr-2"></i>{{ $notification->data['title'] }}
                            <br> {{ $notification->data['user'] }}
                            <span class="float-right text-muted text-sm">{{ $notification->created_at/* ->format('d M Y') */ }}</span>
                        </a>
                    @endforeach
                    <div class="dropdown-divider"></div>
                    <a href="{{ route('admin.orders.MarkAsRead_all') }}" class="dropdown-item dropdown-footer">See All Notifications</a>
                @endif
